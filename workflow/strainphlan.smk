...
if not config["strainphlan_markers_dir"]:
    sys.exit("config 'strainphlan_markers_dir' must be set!  This is where we look for existing marker for a sample, or deposit them if the markers do not exist")
if not os.path.exists(config["strainphlan_markers_dir"]):
    ...
    os.makedirs(os.path.join(config['strainphlan_markers_dir'], "samples"), exist_ok=True)
    os.makedirs(os.path.join(config['strainphlan_markers_dir'], "species"), exist_ok=True)
...
rule all:
    input:
        expand(os.path.join(config['strainphlan_markers_dir'], "samples","{sample}.pkl"), sample=SAMPLES),
        expand(os.path.join(config['strainphlan_markers_dir'], "species","{sp}.fna"), sp=targets),
        expand(
...
rule strainphlan_run:
    input:
        ...
        sample_pkls=expand(os.path.join(config['strainphlan_markers_dir'], "samples","{sample}.pkl"), sample=SAMPLES),
    output:
